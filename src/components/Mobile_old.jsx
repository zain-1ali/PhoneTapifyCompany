...
import { getSingleChild, splitString, getAllChilds } from "../Services";
import {
  setOrgLogo,
...
const Mobile = ({
  ...
  useEffect(() => {
    ...
    conexParent != "superAdmin"  ? getAllChilds(getTeamMembers, false) : getAllChilds(getTeamMembers, false, uid);
  }, []);
    ...
    >
      {leadMode && (
        <div className="absolute w-[100%] flex justify-center items-center h-[455px]">
          <div className="h-[380px] overflow-x-scroll w-[85%] border z-20 rounded-md flex justify-center items-center"
      ...
      <div className="w-[100%] flex flex-col items-center mt-3">
        ...
        </div>
        {
          ...
          {teamMembers?.map((member) => {
            // Check if the member is visible before rendering
            if (visibleMembers?.some((mem) => mem === member.id)) {
              return (
                <div
                  key={member.id} // Add a unique key for React
                  className="w-[35px] h-[50px] flex flex-col items-center"
                >
                  {/* Member Image */}
                  <img
                    src={member.profileUrl ? appendBucketPath(member.profileUrl) : prsnPlshldr}
                    alt={member?.name || "Profile Image"} // Add alt for accessibility
                    className="h-[35px] w-[35px] rounded-[10px] object-cover" // Use Tailwind for styling
                  />
                  {/* Member Name */}
                  <p
                    className="text-[8px] mt-[2px] text-center"
                    style={{
                      color: textColor || "#000000", // Simplified conditional color logic
                    }}
                  >
                    {splitString(member?.name, 15)} {/* Ensure splitString handles undefined names */}
                  </p>
                </div>
              );
            }
            // return null; // Return null if the member is not visible
          })}
          </div>
      </div>
